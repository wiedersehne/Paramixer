...
def build_dataset():
    ...
    df_cs_NE = pd.read_csv("data/cs.NE.csv")
    df_math_AC = pd.read_csv("data/math.AC.csv")
    df_math_GR = pd.read_csv("data/math.GR.csv")
    print(df_cs_AI.head(5))
...
def read_articles():
    train_df = pd.read_csv('long_document_data/train.csv')
    val_df = pd.read_csv('long_document_data/val.csv')
    test_df = pd.read_csv('long_document_data/test.csv')
...
def get_articles_data(max_len):
    ...
    # STEP 3: zero padding
    X_train = tf.keras.preprocessing.sequence.pad_sequences(X_train, maxlen=max_len, value=token_index['<PAD>'],                                                        padding="post")
    ...
    torch.save(X_train, 'long_document16384_train.pt')
    torch.save(y_train, 'long_document16384_train_targets.pt')

    torch.save(X_test, 'long_document16384_test.pt')
    torch.save(y_test, 'long_document16384_test_targets.pt')

    torch.save(X_val, 'long_document16384_val.pt')
    torch.save(y_val, 'long_document16384_val_targets.pt')

    return X_train, y_train, X_test, y_test, X_val, y_val

get_articles_data(16384)
